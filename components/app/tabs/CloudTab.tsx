...
const GLOBAL_CONTROL_HELP = {
  ...
  stopCycleForced:
    'Le mode forcé ne contourne jamais un blocage BUY/SELL, une autorisation de type, une zone, une capacité, le plan, le mode hedging, l’autorisation du compte réel ou AvaBridge 1.70.\nExemple : si BUY est autorisé et SELL bloqué pour une famille, Ava lance uniquement le côté BUY.',
  stopCycleMarket:
...
export function CloudTab({ user, language = 'fr', onGoToSubscription, onSessionExpired }: { user: UserData; language?: string; onGoToSubscription?: () => void; onSessionExpired?: () => void }) {
  ...
  const bridgeOutdated = agentConnected && instance?.bridge_version && Number.isFinite(bridgeVersionNumber) && bridgeVersionNumber < 1.70
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
            ...
            {bridgeOutdated && (
              <div className="mt-5 rounded-2xl border border-amber-400/20 bg-amber-400/[0.08] p-4">
                ...
                <p className="mt-1 text-xs leading-5 text-slate-400">
                  AvaBridge {instance?.bridge_version} est détecté. AvaBridge 1.70 est requis pour fermer immédiatement au bénéfice net configuré après swap, synchroniser Ava S et conserver les protections Stop Cycle. Réinstalle AvaBridge depuis Ava Desktop avant de relancer le moteur.
                </p>
              </div>
            )}
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  const forcedConfirmed = stopCyclePolicy.feature_enabled !== true
                    ...
                    || window.confirm(
                    'Confirmer le mode forcé des cycles conditionnels ? Il restera limité à l’owner, aux comptes MT5 hedging, à AvaBridge 1.70 et à toutes les protections administrateur. Un compte réel exige aussi son autorisation explicite.',
                  )
                  if (!forcedConfirmed) {
